        ...
        int doBJets, int doPUStudy, bool doFlat, bool useRoch, bool doVarWidth,  bool hasPartonInfo, string pdfSet, int pdfMember)
{
    ...
    for (Long64_t jentry(0); jentry < 10; jentry++){
        ...
        if (hasGenInfo) {
            ...
            cout << __LINE__ << " nTotGenLeptons  " << nTotGenLeptons << endl; 
            for (unsigned short i(0); i < nTotGenLeptons; i++) {
                ...
                if ( ( genLepSt_->at(i) == 1 && lepSelector && abs(genLepId_->at(i)) == LeptonID) || ( doW && charge == 0 ) ){
                    ...
                    //cout << genLep.pt << "  " << genLep.eta << endl;
                    if (doW && ((fabs(genLep.charge) > 0 && genLep.pt >= GenMuon1PtMin && fabs(genLep.eta) <= GenMuon1EtaMin) || (fabs(genLep.charge) == 0 && genLep.pt >= GenMuon2PtMin))){
                        cout << __LINE__ << "  there\t" << genLep.pt << "  " << genLep.eta << endl;
                        if(whichW==0) {
                            genLeptons.push_back(genLep); 
            ...
            if (nGenLeptons >= 2){
                ...
                if (doW){
                    ...
                    cout << __LINE__ << passesWhichWGenCut << endl;
                    if (passesWhichWGenCut && (genMT >= GenV_MassMin && genLepton2.pt >= METcut)) {
                      cout << __LINE__ << passesGenLeptonCut << endl;
...
//        //---------
        ...
        //====================================//
        if (hasGenInfo){
            cout << __LINE__ << "\t" << passesGenLeptonCut << "\t" << passesDPSPartonCut << "\t" <<  passesGenJetCut << endl;
            if (passesGenLeptonCut && passesDPSPartonCut && passesGenJetCut){
                ...
                cout << "HERE\t" << genZ.M() << "  " << genZ.Pt() << "  " << genZ.Eta() << "  "
                     << genLep1.Pt() << "  " << genLep1.Eta() << endl;                

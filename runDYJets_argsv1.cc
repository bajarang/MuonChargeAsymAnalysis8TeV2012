...
std::string srcdir = "/grid_mnt/public/bsutar/Asym2016-8TeV/CMSSW_5_3_11/src/TreeAnalysis2012/WJetsp_CENTRAL_QCD0/Sources/";
//gROOT->ProcessLine(string(".L " + srcdir + "ZJetsAndDPS.cc+").c_str());
std::vector<std::string> sources;
//std::string file1 = "getFilesAndHistograms";
...
void runDYJets_argsv1(){
  ...
  //------
  ifstream processVars("ProcessVariables.txt");
  int value;
  int DOWHAT, DOSYSRUNNING, DOQCD, DOBJETS;

  string textline;
  const char *linest;

  for(int ii=0; ii<4; ii++){
    getline(processVars,textline); linest=textline.c_str();
 
    if(textline.find("DOWHAT")!=string::npos){
      sscanf(linest, "%*s %d", &value);
      DOWHAT=value;
    }

    if(textline.find("DOSYSRUNNING")!=string::npos){
      sscanf(linest, "%*s %d", &value);
      DOSYSRUNNING=value;
    }

    if(textline.find("DOQCD")!=string::npos){
      sscanf(linest, "%*s %d", &value);
      DOQCD=value;
    }

    if(textline.find("DOBJETS")!=string::npos){
      sscanf(linest, "%*s %d", &value);
      DOBJETS=value;
    }
  }
  ...
  const unsigned int NSystData(3), NSystMC(12); // set NSystMC to 5 if you want to do only PU, XSEC

...
const string systematicsdata = "0";
const string systematicsmc   = "0";

string appendstring = "";

int UnfoldData(string myStrWCharge, string strSystematics, string myStrDirection, string variable, const TString lepSel, const TString algo, int svdKterm, RooUnfoldResponse *resp, TH1D *hRecDataMinusFakes, TH1D* &hUnfData, TString name);
...
//void myFastPlots(string leptonFlavor, string strVariable, bool closureTest)
void myFastPlots(string leptonFlavor, string strWCharge, string strVariable, string strClosureTest, string strSystematics, string strDirection, string strRange)
{
    ...
    if(!strRange.empty()) strVariable = strVariable + "_leppt_" + strRange;

    bool closureTest = false;
    if(strClosureTest=="1") closureTest = true;
    ...
    if (strVariable == ""){
        ...
        for (int i(40); i < 50; i++){
            ...
            if (!isMuon) FastPlotsRun(sel, nsel, strWCharge, strSystematics, strDirection, leptonFlavor, varStruct[i].name, varStruct[i].log, varStruct[i].decrease, varStruct[i].ESVDkterm, varStruct[i].EBayeskterm);
            if (isMuon)  FastPlotsRun(sel, nsel, strWCharge, strSystematics, strDirection, leptonFlavor, varStruct[i].name, varStruct[i].log, varStruct[i].decrease, varStruct[i].MuSVDkterm, varStruct[i].MuBayeskterm);
        ...
        if(varList->Contains(strVariable.c_str())){
            TObjString *name = new TObjString(strVariable.c_str());
            unsigned short j(varList->IndexOf(name));
            cout << setw(3) << " " << ") Processing variable: " << varStruct[j].name << endl; 
            if (!isMuon) FastPlotsRun(sel, nsel, strWCharge, strSystematics, strDirection, leptonFlavor, varStruct[j].name, varStruct[j].log, varStruct[j].decrease, varStruct[j].ESVDkterm, varStruct[j].EBayeskterm, closureTest);
            if (isMuon)  FastPlotsRun(sel, nsel, strWCharge, strSystematics, strDirection, leptonFlavor, varStruct[j].name, varStruct[j].log, varStruct[j].decrease, varStruct[j].MuSVDkterm, varStruct[j].MuBayeskterm, closureTest);
        }
        else{
            if (!isMuon) FastPlotsRun(sel, nsel, strWCharge, strSystematics, strDirection, leptonFlavor, strVariable.c_str(),1,1,5,5,closureTest);
            if (isMuon)  FastPlotsRun(sel, nsel, strWCharge, strSystematics, strDirection, leptonFlavor, strVariable.c_str(),1,1,5,5,closureTest);
        }
    }
}
...
void FastPlotsRun(const int *sel, int nsel, string strWCharge, string strSystematics, string strDirection, string leptonFlavor, string variable, bool logZ, bool decrease, int SVDkterm, int Bayeskterm, bool closureTest, int JetPtMin, int JetPtMax, bool doFlat, bool doVarWidth)
{
    ...
    int charge = 0;
    strStreamWCharge >> charge;
    string myStrWCharge;
    if     (charge==1)  myStrWCharge = "WP";
    else if(charge==-1) myStrWCharge = "WM";

    // parse strSystematics
    ...
    int direction = 0;
    strStreamDirection >> direction;
    string myStrDirection;
    if     (direction==0)  myStrDirection = "CN";
    else if(direction==1)  myStrDirection = "UP";
    else if(direction==-1) myStrDirection = "DN";
    ...
    if (!closureTest){
      ...
      TH1D *hUnfData= NULL;
      int nIter = UnfoldData(myStrWCharge, strSystematics, myStrDirection, variable, leptonFlavor, "Bayes", Bayeskterm, responseZJ, hRecDataMinusFakes, hUnfData, "");
      outputRootFile->cd();
      hUnfData->Write();
    
      TH1D *hUnfDataSVD= NULL;
      int nIterSVD = UnfoldData(myStrWCharge, strSystematics, myStrDirection, variable, leptonFlavor, "SVD", Bayeskterm, responseZJ, hRecDataMinusFakes, hUnfDataSVD, "");
    }
    ...
    string CompareAlgoOutFileName = OutDirAlgo + "/" + variable + "_CompareAlgo.pdf";
...
int UnfoldData(string myStrWCharge, string strSystematics, string myStrDirection, string variable, const TString lepSel, const TString algo, int svdKterm, RooUnfoldResponse *resp, TH1D *hRecDataMinusFakes, TH1D* &hUnfData, TString name)
{
    ...
    TFile *f; // = new TFile ("delete/delete.root","RECREATE");
    
    f = new TFile(lepSel + "_" + variable + "_" + name + "_" + algo + ".root", "RECREATE");
    f->cd();
    int finalNIter = -1;
    int nIter = 99;
    ...
    hchi2->Write();
    chchi2->Write();
    chchi2->Print(unfCheckDir + lepSel + "_" + variable + "_" + name + "_" + algo + "_chi2.pdf");
